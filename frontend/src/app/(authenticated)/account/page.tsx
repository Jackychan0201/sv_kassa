...
export default function AccountPage() {
  ...
  if (!user || !user.user) {
    return <LoadingFallback message="Loading user..." />;
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Build payload according to UpdateShopDto
      const body: Record<string, any> = {};
      if (name !== user.user.name) body.name = name;
      if (email !== user.user.email) body.email = email;
      if (password.trim() !== "") body.password = password;

      const res = await fetch(`/api/shops/${user.user.shopId}`, {
        method: "PATCH",
      ...
      user.setEmail(body.email ?? email);
      user.setName(body.name ?? name);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-y-4">
        <Label className="text-xl">Name: {user.user.name}</Label>
        <Label className="text-xl">Email: {user.user.email}</Label>
      </div>
